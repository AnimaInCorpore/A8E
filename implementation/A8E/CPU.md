...
- Notes: opcode handling and flags are cycle-driven and act as base timing for other chips. NMI dispatch accepts one pending edge while a handler is active, and POKEY IRQ delivery follows the active level instead of retaining disabled sources. The fake6502-compatible undocumented opcode set now covers `ANE`/`LXA` plus `ARR`/`LAS`/`SHA`/`SHX`/`SHY`/`TAS`/`RRA`/`SBX`, including the `SHX`/`SHY` store-address quirk and the `RRA`/`ISC` decimal-cycle cancel so the Lorenz opcode suite matches the upstream reference behavior.
- Issues: none tracked.
