...
- Notes: WebGL uses a two-pass path (decode pass, then CRT post-process pass). CRT shader applies filtering and scanline shaping; software rendering is fallback if WebGL/shaders fail. Frame presentation is now coalesced to one paint per browser RAF callback even when the emulation loop catches up across multiple emulated frames, and WebGL contexts avoid low-latency/desynchronized mode to reduce visible tearing during frame delivery.
