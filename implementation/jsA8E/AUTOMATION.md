...
- Files: `jsA8E/js/app/automation_api.js`, `jsA8E/js/app/automation/utils.js`, `jsA8E/js/app/ui.js`, `jsA8E/js/core/atari.js`, `jsA8E/js/core/debugger.js`, `jsA8E/js/core/memory.js`, `jsA8E/js/core/input.js`, `jsA8E/js/core/hostfs.js`, `jsA8E/js/core/app_proxy.js`, `jsA8E/emulator_worker.js`, `jsA8E/js/core/assembler_core.js`, `jsA8E/js/core/snapshot_codec.js`
...
- Status: verified on 2026-03-11 (`implemented`, with follow-up UI/tooling ideas below).
- Notes: `window.A8EAutomation` now exposes the planned browser-first grouped surface (`system`, `media`, `input`, `debug`, `dev`, `artifacts`, `events`) while keeping the earlier flat aliases for compatibility. The live API covers worker-safe request/response RPC, deterministic paused-mode debug execution, explicit stop/fault semantics, cross-realm-safe binary normalization for automation inputs, URL-native media loaders (`loadRomFromUrl`, `loadOsRomFromUrl`, `loadBasicRomFromUrl`, `mountDiskFromUrl`, `runXexFromUrl`) with shared cache/version fetch controls, progress events, memory/bank introspection, trace/counter export, framebuffer-based screenshot/artifact capture, schema-versioned failure snapshots, HostFS access, source-text assembly, XEX launch helpers, wait primitives, mapped source context, runtime disassembly, reset-time boot overrides such as `portB` for pre-boot banking control, and paused-state snapshot save/load (`system.saveSnapshot`, `system.loadSnapshot`) backed by the core runtime's versioned binary snapshot container. Generic binary/fetch/error helpers now live in `js/app/automation/utils.js`, which trims `automation_api.js` down to the public facade and domain-specific orchestration. Worker-backed lifecycle calls (`start`, `pause`, `reset`) now resolve only after the worker acknowledges the state transition, regular disk mounts prefer the detailed request/response path when available, and `getSystemState({ timeoutMs })` now uses bounded per-part reads so stalled worker calls degrade into partial state with structured `error.details.parts` instead of hanging forever. `dev.runXex(...)` now emits an explicit `xex_mount_started` checkpoint before worker-backed XEX mount/preflight work, awaits reset/start acknowledgements, and still drives entry waits through paused `runUntilPc(...)` so loader/preflight faults and tight boot loops come back as structured `xex_boot_failed` artifacts with explicit boot diagnostics instead of falling through to generic timeout snapshots. The browser UI attaches the live app instance, but the public contract lives in `automation_api.js`, not in UI internals.
...
window.A8EAutomation = {
  ...
  media: {
    ...
    loadOsRom(data),
    loadBasicRom(data),
    loadRomFromUrl(kind, url, options),
    loadOsRomFromUrl(url, options),
    loadBasicRomFromUrl(url, options),
...
- `getCapabilities()`: boolean/enum feature matrix such as `worker`, `hostfs`, `assembler`, `trace`, `screenshot`, `disk`, `urlRomLoad`, `urlDiskLoad`, `urlXexLoad`, `joystick`, `consoleKeys`.
