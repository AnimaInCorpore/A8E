...
- 2026-03-20: `jsA8E/js/core/playfield/mode_8_f.js`, `jsA8E/tests/playfield_mode_8_f_rendering.test.js`, `jsA8E/package.json`: fixed the remaining double-width regression in ANTIC Mode F PRIOR mode 0 rendering. The JS path had been writing paired pixels for each bit in the single-color case, while the native renderer writes one pixel per bit; Mode F now uses single-pixel writes in that branch, and a regression test checks that a 1-byte scanline stops at the expected 8-pixel boundary.

- 2026-03-20: `jsA8E/js/core/playfield/mode_2_3.js`, `jsA8E/tests/playfield_mode_2_3_rendering.test.js`: fixed a width regression in ANTIC Mode 2/3 PRIOR mode 0 rendering. The recent pixel-writing refactor had expanded text output with `writePixelPair()`, which doubled the visible width; Mode 2/3 now writes one pixel per text bit again, and a regression test now checks that the scanline stops at the expected pixel count.
