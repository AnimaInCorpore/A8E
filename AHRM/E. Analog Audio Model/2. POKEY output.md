...
The output from the four channel DACs is added, but the result is subject to a non-linear saturation effect as the total volume level rises. The output is roughly linear up to a total volume level of 12 before compression starts to become visible in the signal, and distortion becomes audible at around total levels of 30 or higher.
...
```math
y = \begin{cases} 2.171 x & x \leq 0.14 \\ 2.171 (0.14 + \dfrac{1.0 - e^{-2.85 * (x - 0.14)}}{2.85}) & x \geq 0.14 \end{cases}
```
