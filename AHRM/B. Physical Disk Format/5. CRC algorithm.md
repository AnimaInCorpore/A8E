...
```
def crc16(data:bytes, initial_value:int = 0xFFFF) -> int: crc = initial_value # for each data byte for c in data: # add in next byte crc ^= c << 8 # compute crc*2^8 mod P for i in range(0,8): # multiply CRC by 2 crc <<= 1 # modulo by polynomial
```

```
if crc & 0x10000: crc ^= 0x11021 return crc
```
...
```
crc16_table = [crc16(bytes([x]), 0) for x in range(0,256)] def crc16_fast(data:bytes, initial_value:int = 0xFFFF) -> int: crc = initial_value for c in data: crc = ((crc << 8) & 0xFFFF) ^ crc16_table[c ^ (crc >> 8)] return crc
```

This CRC algorithm is also available as the crc\_hqx() function in the binascii module:

```
>>> import binascii >>> sd_sector1 = b'\xFE\x00\x00\x01\x00' >>> ed_sector1 = b'\xA1\xA1\xA1\xFE\x00\x00\x01\x00' >>> '{:02X}'.format(binascii.crc_hqx(sd_sector1, 0xFFFF)) 'D2C3' >>> '{:02X}'.format(binascii.crc_hqx(ed_sector1, 0xFFFF)) 'EA2D'
```
