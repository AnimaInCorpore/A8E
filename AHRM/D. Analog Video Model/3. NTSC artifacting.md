...
There are a couple of ways to improve the quality of the separated luma. First, the luma and chroma filters should be matched so that there is no signal loss between them. This is most easily accomplished by subtracting the chroma from the composite signal to produce luma, equivalent to ensuring that the luma and chroma filters are perfect inverses.<sup>93</sup> Second, a post-separation peaking filter can be used that has increased gain around 3.58MHz, to combat attenuation of frequency components around the chroma subcarrier. This is a common option in NTSC decoding chips.
...
\begin{array}{lcl}
R - Y & = & 0.956 I + 0.621 Q \\
 & = & 0.956 \cos(2 \pi f_{sc} t) + 0.621 \sin(2 \pi f_{sc} t) \\
 & = & 1.140 \cos(2 \pi f_{sc} t + 33.0^{o}) \\
G - Y & = & -0.272 I - 0.647 Q \\
 & = & -0.272 \cos(2 \pi f_{sc} t) - 0.647 \sin(2 \pi f_{sc} t) \\
 & = & 0.493 \cos(2 \pi f_{sc} t - 123.5^{o}) \\
B - Y & = & -1.107 I - 1.704 Q \\
 & = & -1.107 \cos(2 \pi f_{sc} t) - 1.704 \sin(2 \pi f_{sc} t) \\
 & = & 2.032 \cos(2 \pi f_{sc} t + 123.0^{o})
\end{array}
```
...
```math
\begin{array}{lcl}
signal & = & (0.299 R + 0.587 G + 0.114 B) \\
 & & +(0.596 R - 0.275 G - 0.321 B) \cos(2 \pi f_{sc} t) \\
 & & +(0.212 R - 0.523 G + 0.311 B) \sin(2 \pi f_{sc} t) \\
 & = & R (0.299 + 0.596 \cos(2 \pi f_{sc} t) + 0.212 \sin(2 \pi f_{sc} t)) \\
 & & +G (0.587 - 0.275 \cos(2 \pi f_{sc} t) - 0.523 \sin(2 \pi f_{sc} t)) \\
 & & +B (0.321 - 0.321 \cos(2 \pi f_{sc} t) + 0.311 \sin(2 \pi f_{sc} t)) \\
 & = & R \cdot (0.299 + 0.633 \cos(2 \pi f_{sc} t - 19.5^{o})) \\
 & & +G \cdot (0.587 + 0.591 \cos(2 \pi f_{sc} t - 117.7^{o})) \\
 & & +B \cdot (0.114 + 0.447 \cos(2 \pi f_{sc} t + 135.9^{o}))
\end{array}
```
...
All filtering operations must take place in NTSC gamma space and not linear color, since this is how NTSC encoding is specified.
