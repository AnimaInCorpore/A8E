...
```math
\begin{bmatrix} U \\ V \end{bmatrix} = \begin{bmatrix} -\sin 33^{\circ} & \cos 33^{\circ} \\ \cos 33^{\circ} & \sin 33^{\circ} \end{bmatrix} \begin{bmatrix} I \\ Q \end{bmatrix}
```
...
```math
\begin{array}{ll}
\begin{bmatrix} R \\ G \\ B \end{bmatrix} = \begin{bmatrix} 1.164 & 0 & 1.596 \\ 1.164 & -0.392 & -0.813 \\ 1.164 & 2.017 & 0 \end{bmatrix} \begin{bmatrix} Y - 16 \\ Cb - 128 \\ Cr - 128 \end{bmatrix} & \text{(Rec. 601 limited range (SD))} \\
\begin{bmatrix} R \\ G \\ B \end{bmatrix} = \begin{bmatrix} 1 & 0 & 1.402 \\ 1 & -0.344 & -0.714 \\ 1 & 1.772 & 0 \end{bmatrix} \begin{bmatrix} Y \\ Cb - 128 \\ Cr - 128 \end{bmatrix} & \text{(Rec. 601 full range (JPEG JFIF))}
\end{array}
```
...
<table>
<tr><th rowspan="2">Element</th><th colspan="4">Signal levels (+/-0.008V)</th></tr>
<tr><th>800XL</th><th>130XE</th><th>800</th><th>400<sup>92</sup></th></tr>
<tr><td>Sync</td><td>0.580V</td><td>0.044V</td><td>0.080V</td><td>3.604V</td></tr>
<tr><td>Luma 0</td><td>1.112V</td><td>0.468V</td><td>0.640V</td><td>3.736V</td></tr>
<tr><td>Luma 1</td><td>1.184V</td><td>0.532V</td><td>0.696V</td><td>3.752V</td></tr>
<tr><td>Luma 2</td><td>1.264V</td><td>0.596V</td><td>0.744V</td><td>3.768V</td></tr>
<tr><td>Luma 3</td><td>1.334V</td><td>0.660V</td><td>0.800V</td><td>3.780V</td></tr>
<tr><td>Luma 4</td><td>1.408V</td><td>0.716V</td><td>0.856V</td><td>3.792V</td></tr>
<tr><td>Luma 5</td><td>1.488V</td><td>0.780V</td><td>0.904V</td><td>3.808V</td></tr>
<tr><td>Luma 6</td><td>1.568V</td><td>0.852V</td><td>0.960V</td><td>3.820V</td></tr>
<tr><td>Luma 7</td><td>1.648V</td><td>0.908V</td><td>1.016V</td><td>3.836V</td></tr>
<tr><td>Luma 8</td><td>1.688V</td><td>0.940V</td><td>1.056V</td><td>3.848V</td></tr>
<tr><td>Luma 9</td><td>1.760V</td><td>1.004V</td><td>1.112V</td><td>3.864V</td></tr>
<tr><td>Luma 10</td><td>1.840V</td><td>1.068V</td><td>1.160V</td><td>3.876V</td></tr>
<tr><td>Luma 11</td><td>1.928V</td><td>1.132V</td><td>1.224V</td><td>3.892V</td></tr>
<tr><td>Luma 12</td><td>1.992V</td><td>1.196V</td><td>1.272V</td><td>3.904V</td></tr>
<tr><td>Luma 13</td><td>2.072V</td><td>1.260V</td><td>1.328V</td><td>3.920V</td></tr>
<tr><td>Luma 14</td><td>2.144V</td><td>1.324V</td><td>1.384V</td><td>3.936V</td></tr>
<tr><td>Luma 15</td><td>2.224V</td><td>1.380V</td><td>1.440V</td><td>3.952V</td></tr>
</table>
...
```math
\begin{bmatrix} I \\ Q \end{bmatrix} = saturation \times \begin{bmatrix} \cos(-57^{\circ} + delay \times (hue - 1)) \\ \sin(-57^{\circ} + delay \times (hue - 1)) \end{bmatrix}
```

Or, equivalently, in U-V:

```math
\begin{bmatrix} U \\ V \end{bmatrix} = saturation \times \begin{bmatrix} \cos(180^{\circ} - delay \times (hue - 1)) \\ \sin(180^{\circ} - delay \times (hue - 1)) \end{bmatrix}
```
...
```math
\begin{array}{l}
\theta_{\textit{R-Y}} = \mathit{atan2}(1.140, 0) = 90^{\circ} \\
\theta_{\textit{G-Y}} = \mathit{atan2}(-0.581, -0.395) = 235.8^{\circ} - 360^{\circ} \\
\theta_{\textit{B-Y}} = \mathit{atan2}(0, 2.032) = 0^{\circ}
\end{array}
```
...
```math
\begin{array}{l}
gain_{\textit{R-Y}} = \left| (1.140, 0) \right| = 1.140 \\
gain_{\textit{G-Y}} = \left| (-0.581, -0.395) \right| = 0.703 \\
gain_{\textit{B-Y}} = \left| (0, 2.032) \right| = 2.032
\end{array}
```
...
```math
\begin{array}{l}
gain_{\textit{G-Y}} / gain_{\textit{R-Y}} = 0.703 / 1.140 = 0.617 \\
gain_{\textit{B-Y}} / gain_{\textit{R-Y}} = 2.032 / 1.140 = 1.782 \\
gain_{\textit{R-Y}} / gain_{\textit{B-Y}} = 1.140 / 2.032 = 0.561 \\
gain_{\textit{G-Y}} / gain_{\textit{B-Y}} = 0.703 / 2.032 = 0.346
\end{array}
```
...
```math
\left( k \times rgb^{gamma} \right)^{(1/gamma)} = k^{(1/gamma)} \times \left( rgb^{gamma} \right)^{(1/gamma)} = k^{(1/gamma)} \times rgb
```
...
v_{avg} = \left( \frac{\left( v_{lo}^{2.2} + v_{hi}^{2.2} \right)}{2} \right)^{\frac{1}{2.2}} = 0.685
\end{gathered}
```
