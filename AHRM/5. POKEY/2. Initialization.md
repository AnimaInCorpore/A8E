...
Both clocks are polynomial counters with truncated cycles. The 15KHz clock is a 7-bit XNOR counter with a polynomial of x<sup>7</sup>+x<sup>6</sup>+1. On a pattern of %1001001 (shifting left), a '1' bit is forced and a pulse emitted. This occurs 78 cycles after the reset state of %0000000. The 64KHz clock is a 5-bit XOR counter with polynomial x<sup>5</sup>+x<sup>3</sup>+1 and a forced '1' bit on %00010, occurring 19 cycles after the reset state of %11111. If IRQs are enabled for unlinked timers using these clocks with period 0, the IRQ is asserted in IRQST 83 and 24 cycles after the write to SKCTL that clears init mode.
