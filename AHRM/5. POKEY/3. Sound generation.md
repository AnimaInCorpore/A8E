...
the output of this generator every P cycles, or equivalently advance within the pattern every (P mod N) bits. If the period is divisible by N, the same bit in the pattern will be sampled each time and no sound will occur. With the 4bit generator, N=15, and thus this occurs whenever the period is a multiple of 15 cycles.
