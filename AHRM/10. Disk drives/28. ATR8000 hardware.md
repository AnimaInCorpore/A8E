...
Output operations to ports 20-2FH write to the parallel printer port data latch. This data is then sent to the printer on the next data strobe, which is accomplished by raising and then lowering bit 0 on port 53H/5BH. Ports 20-2FH can also be read to query printer status, particularly the BUSY line. Table 93 gives the layout of this port.
