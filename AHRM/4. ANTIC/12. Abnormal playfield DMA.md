...
The pertinent effect of horizontal scrolling is that the playfield DMA pattern is delayed by one clock for every increase in the HSCROL value by 2. This shifts the playfield DMA start and stop points, but notably doesn’t affect the timing of the DMA clock once it has already started. This means that if the DMA stop position can be moved to a position that doesn’t stop the DMA clock.
...
| Start of line | After playfield DMA start | After playfield DMA stop |
|---|---|---|
